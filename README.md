...
Originally built for a Susi Air technical assessment, then extended with the AI copilot, logbook, motion/accessibility polish, a Remote Ops Briefing design system, and a transparent demo-provenance layer.
...
- **Sign In** (`/`) — demo-only; any input navigates to `/home` (no real authentication). A visible **Demo mode** alert upfront sets honest expectations.
...
- **Schedule** (`/schedule`) — month calendar grid with duty badges rendered from legend data (never hardcoded codes), tap-a-date detail modal, legend, data freshness header with viewing-month context.
...
1. **Unit specs** (Vitest + happy-dom) — pure-function tests for the composables. `useRollingSum.spec.ts` hand-verifies the ±7-day rolling window across all chart points. `brief-verification.spec.ts` is a cross-cutting canary asserting the assessment's worked examples.
